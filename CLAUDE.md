...
After `migrate` (which enables the pgvector extension and creates `note_en_embedding`/`note_fr_embedding`), semantic search won't return anything for rows that don't have embeddings yet — backfill them with:
...
Run this again any time new images are loaded/fixtures reloaded (`make load-fixtures` etc.), since new rows won't have embeddings until this runs.
...
- **Image derivatives**: `Image` stores three separate `ImageField`s — `file` (often the original high-res TIFF, not browser-renderable), `thumbnail` (browser-safe preview), and `zoom`. Admin preview and most public rendering prefer `thumbnail` over `file` for this reason (see `images/admin.py: img_preview`). Thumbnails are generated by `scripts/data/generate_thumbnails.py`, not on save.
...
- **Semantic search / embeddings**: `Image.note_en_embedding`/`note_fr_embedding` (`pgvector.django.VectorField`, 384 dims, `HnswIndex` with `vector_cosine_ops`) hold sentence embeddings of the translated notules, generated via `sentence-transformers` (`paraphrase-multilingual-MiniLM-L12-v2`, see `images/embeddings.py`). They're populated by the `generate_embeddings` management command, not on save — see Commands above. Requires the `pgvector` Postgres extension (enabled by migration `0011_enable_pgvector`); `docker-compose.yaml`'s `db` service uses the `pgvector/pgvector:pg15` image for this.
